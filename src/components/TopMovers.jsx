  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0009", zIndex: 1000 }}>
      <div style={{ width: "min(520px, 92vw)", margin: "10% auto", background: "#1f1f1f", borderRadius: 8, color: "#fff", padding: "1.25rem 1.25rem 1rem" }}>
        ...
        <input type="number" min="1" step="1" value={amount} onChange={(e) => onChangeAmount(e.target.value)} style={{ width: "100%", padding: "10px 12px", borderRadius: 6, border: "1px solid #333", background: "#121212", color: "#fff" }} placeholder="Ex: 1000" />
...
const TopMovers = () => {
  const { currentPrices, positionsMap, buyPosition, sellPosition } = useContext(PortfolioContext);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (cancel) return;
        setTopGainers(Array.isArray(data?.gainers) ? data.gainers : []);
        setTopLosers(Array.isArray(data?.losers) ? data.losers : []);
  ...
  const openBuy = (sym) => { setBuySymbol(sym.toUpperCase()); setBuyAmount(""); setShowBuy(true); };
  ...
  const openSell = (sym) => { setSellSymbol(sym.toUpperCase()); setSellPercent(100); setShowSell(true); };
  const confirmSell = async () => { await sellPosition(sellSymbol, sellPercent); setShowSell(false); };
  ...
  const renderRows = (rows) =>
    rows.map((r, i) => {
      ...
      const price = Number.isFinite(r?.price) ? r.price : currentPrices[sym] ?? NaN;
      const chg = Number.isFinite(r?.change5m) ? r.change5m : Number.isFinite(r?.change) ? r.change : null;
      return (
        <tr key={`${sym}-${i}`} style={{ background: i % 2 ? "#222" : "#262626" }}>
          <td style={tdStyle}><b>{sym}</b></td>
          <td style={tdStyle}>{fmtPrice(price)}</td>
          <td style={{ ...tdStyle, color: typeof chg === "number" ? (chg >= 0 ? "lightgreen" : "salmon") : "#ddd" }}>{typeof chg === "number" ? fmtPct(chg) : "—"}</td>
          <td style={tdStyle}>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => openBuy(sym)} style={{ padding: "6px 10px", border: "none", borderRadius: 4, background: "#198754", color: "#fff", cursor: "pointer", fontWeight: 600 }}>ACHAT</button>
              {hasOpen(sym) && <button onClick={() => openSell(sym)} style={{ padding: "6px 10px", border: "none", borderRadius: 4, background: "#dc3545", color: "#fff", cursor: "pointer", fontWeight: 600 }}>VENTE</button>}
            </div>
          </td>
          <td style={tdStyle}>{hasOpen(sym) ? <span style={{ color: "#dc3545" }}>●</span> : <span style={{ color: "#888" }}>○</span>}</td>
          <td style={tdStyle}>
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 16, alignItems: "baseline", marginBottom: 10 }}>
        ...
        <span style={{ color: "#aaa", fontSize: 12 }}>{lastTs ? `MAJ ${new Date(lastTs).toLocaleTimeString()}` : ""}</span>
      </div>

      {loadErr ? (
        <div style={{ background: "#2a2a2a", color: "#fff", padding: 10, borderRadius: 8, marginBottom: 12, border: "1px solid #3a3a3a" }}>{loadErr}</div>
      ) : null}
      ...
      <BuyModal show={showBuy} symbol={buySymbol} amount={buyAmount} onChangeAmount={setBuyAmount} onClose={() => setShowBuy(false)} onConfirm={confirmBuy} />
      <SellModal
        show={showSell}
        symbol={sellSymbol}
