...
// src/constants.ts
var FILE_SW_REGISTER, VIRTUAL_MODULES_MAP, VIRTUAL_MODULES_RESOLVE_PREFIX, VIRTUAL_MODULES, defaultInjectManifestVitePlugins, PWA_INFO_VIRTUAL, RESOLVED_PWA_INFO_VIRTUAL, PWA_ASSETS_HEAD_VIRTUAL, RESOLVED_PWA_ASSETS_HEAD_VIRTUAL, PWA_ASSETS_ICONS_VIRTUAL, RESOLVED_PWA_ASSETS_ICONS_VIRTUAL, DEV_SW_NAME, DEV_SW_VIRTUAL, RESOLVED_DEV_SW_VIRTUAL, DEV_READY_NAME, DEV_REGISTER_SW_NAME, DEV_PWA_ASSETS_NAME;
var init_constants = __esm({
  "src/constants.ts"() {
    "use strict";
    FILE_SW_REGISTER = "registerSW.js";
    VIRTUAL_MODULES_MAP = {
    ...
    VIRTUAL_MODULES_RESOLVE_PREFIX = "/@vite-plugin-pwa/";
    VIRTUAL_MODULES = Object.keys(VIRTUAL_MODULES_MAP);
    ...
    PWA_INFO_VIRTUAL = "virtual:pwa-info";
    RESOLVED_PWA_INFO_VIRTUAL = `\0${PWA_INFO_VIRTUAL}`;
    PWA_ASSETS_HEAD_VIRTUAL = "virtual:pwa-assets/head";
    RESOLVED_PWA_ASSETS_HEAD_VIRTUAL = `\0${PWA_ASSETS_HEAD_VIRTUAL}`;
    PWA_ASSETS_ICONS_VIRTUAL = "virtual:pwa-assets/icons";
    RESOLVED_PWA_ASSETS_ICONS_VIRTUAL = `\0${PWA_ASSETS_ICONS_VIRTUAL}`;
    DEV_SW_NAME = "dev-sw.js?dev-sw";
    DEV_SW_VIRTUAL = `${VIRTUAL_MODULES_RESOLVE_PREFIX}pwa-entry-point-loaded`;
    RESOLVED_DEV_SW_VIRTUAL = `\0${DEV_SW_VIRTUAL}`;
    DEV_READY_NAME = "vite-pwa-plugin:dev-ready";
    DEV_REGISTER_SW_NAME = "vite-plugin-pwa:register-sw";
    DEV_PWA_ASSETS_NAME = "vite-plugin-pwa:pwa-assets";
  }
});
...
var import_promises, import_generate_assets;
...
async function checkFileExists(pathname) {
  try {
    await (0, import_promises2.access)(pathname, import_node_fs8.default.constants.R_OK);
  } catch {
    return false;
  }
  return true;
...
async function tryToResolveImage(root, sources, image) {
  const imagePath = (0, import_node_path9.resolve)(root, image);
  if (await checkFileExists(imagePath)) {
    return imagePath;
  }
  for (const source of sources) {
    const sourceImage = (0, import_node_path9.resolve)((0, import_node_path9.dirname)(source), image);
...
var import_node_fs8, import_promises2, import_node_path9, import_instructions, import_config;
...
function buildManifestEntry(publicDir, url) {
  return new Promise((resolve8, reject) => {
    ...
    const stream = import_node_fs.default.createReadStream((0, import_node_path.resolve)(publicDir, url));
    stream.on("error", (err) => {
      reject(err);
    });
...
init_log();
var import_meta = {};
var _dirname = typeof __dirname !== "undefined" ? __dirname : (0, import_node_path4.dirname)((0, import_node_url.fileURLToPath)(import_meta.url));
...
function createContext(userOptions) {
  const _dirname2 = typeof __dirname !== "undefined" ? __dirname : (0, import_node_path6.dirname)((0, import_node_url2.fileURLToPath)(import_meta2.url));
  const { version } = JSON.parse(
    (0, import_node_fs5.readFileSync)((0, import_node_path6.resolve)(_dirname2, "../package.json"), "utf-8")
  );
  return {
    version,
    userOptions,
